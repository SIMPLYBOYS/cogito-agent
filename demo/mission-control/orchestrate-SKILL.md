...
2. **分派**：
   ...
   - ⚠️ **給路徑，不要貼內容**：子 agent 與你**共用同一個工作區**，它自己讀得到檔案。
     `task_prompt` 寫「審查 `tools/foo.py`」就好，不要把整份原始碼複製進去——
     貼進去等於同一份內容付兩次輸入費，而且擠掉子 agent 自己探索的空間。
     只有「你腦中才有、檔案裡沒有」的東西（決策脈絡、審查意見、前一個 agent 的結論）
     才需要寫進 `task_prompt`。
...
- **簡報要短**：實測 30 次委派的中位數是 687 字元，但最長的 4 次超過 2000——全是把整份原始碼貼進 `task_prompt`。那 4 次佔了總重新交代量的一半以上。
