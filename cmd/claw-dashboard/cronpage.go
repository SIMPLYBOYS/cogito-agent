...
{{range .Jobs}}
  <div class="cronitem">
    ...
    <details class="danger"><summary>移除</summary>
      <div class="confirm">
        <span>確定移除排程任務「<b>{{.Name}}</b>」？設定與執行紀錄一併刪除，無法復原。</span>
        <form method="POST" action="/cron/remove"><input type="hidden" name="id" value="{{.ID}}"><button type="submit" class="gact">確定移除</button></form>
      </div>
    </details>
