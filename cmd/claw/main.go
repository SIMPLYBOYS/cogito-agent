...
func main() {
	...
	// 高危操作審批 middleware：命中黑名單（如 bash rm -r / sudo / 覆蓋 .go）的工具調用會被掛起，
	// 把審批請求推回觸發它的 Slack 頻道（session.ID == channelID），等管理員回 approve/reject 才放行。
	// 抽成函式以便主工具池與子智能體只讀池共用（子 agent 的 bash 同樣要過審批，不留後門）。
	approval := func(ctx context.Context, call schema.ToolCall) (bool, string) {
