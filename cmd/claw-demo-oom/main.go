// cmd/claw-demo-oom 是上下文壓縮（OOM 防線）演示：用一個會讀入巨型檔案的
// 三步任務，觸發 engine 內置的 Compactor，讓"字元級壓縮"行為眼見為憑。
...
func main() {
	...
	line := "這是一段極其冗長的、無意義的伺服器報錯日誌訊息，用來模擬 OOM 場景\n"
	bigLog := strings.Repeat(line, 200) // ~200 行 ≈ 12KB，遠超 3000 字元閾值
