...
- **多個 OpenAI 相容端點同時使用**（2026-09-17 GPT-6 review）：非 `claude-` 模型全走同一組
  `OPENAI_BASE_URL`／`OPENAI_API_KEY`，無法同時接 OpenAI、OpenRouter 與自架 vLLM；模型 id 也表達不了
  端點身分。升級路徑是具名端點設定（模型寫成 `<端點>/<模型>`，路由收斂在 `provider.ForModel` 與兩個
  `Configure`）。**觸發**：真的要在同一個部署裡並用第二個相容端點時。現況單一端點夠用。
- **每個端點的全域並發上限**（同上 review）：並發控制只有同工作目錄的行程內鎖，頻道一多，請求會一起
  打到同一端點；多行程共用工作目錄時這把鎖也協調不了（現行設計是「一個員工一個目錄」，本就不共用）。
  429 已有帶擾動的退避重試（`backoffDelay`）。升級路徑是依端點的計數信號量排隊。**觸發**：log 出現
  重試用盡仍 429 而任務失敗（`HTTP 429，退避後重試 3/3` 之後報錯），或要水平擴成多行程共用工作目錄時。

---

## 🚫 **判斷過不做**的（對照 Hermes 的短板，以及評估過的外部工具）
...
> **Ragas 的翻盤條件**：當 cogito 的主要用法從「動手做事」變成「拿一份文件庫回答問題」時——那時品質主軸才會變成「答得忠不忠實」，faithfulness 才值那個錢。以現在的用法（IM 派任務、跑指令、改檔、產文件）沒到。
> 另：它的 relevancy **反向技巧**（拿答案反推「什麼問題會得到這個答案」再跟原題比對）值得記著，要做「有沒有離題」時用 Go 對 session 檔算即可——拿想法，不拿包。
