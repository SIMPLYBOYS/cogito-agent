...
>
> **2026-08-05：第一項已結案。** 順帶把顯著性檢定寫進工具（`internal/eval/abstats.go` +
> `-ab-n` 旗標）——先前 p=0.206 是手算的，跨 20 次 stdout 手動計票只會再犯一次同類方法論錯誤。
> 補樣本推翻了 n=5 的三個幅度估計，詳見 [eval-results.md](eval-results.md) §③。
