...
> **狀態：研究，未實作、未排期。** 這份不是待辦——它拆解 Hermes Agent v0.20.0 的 Kanban
> 設計，用來**重新檢查我們自己那條待辦的題目對不對**。結論是題目可能設錯了，但重寫題目
> 也還不到動手的時候。待辦在 [roadmap-next.md](roadmap-next.md)，記憶層現況在
...
**orchestrator 實跑、確認「每次從零開始」真的痛。** 這份研究改的是「痛的時候要做什麼」，
不是「現在就該做」。在那之前，這份文件的用途是：**下次有人說「我們該做 shared memory」，
先問一句——你要的是共享事實，還是共享工作狀態？**
