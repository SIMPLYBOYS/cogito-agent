...
慣例進 [DESIGN.md](../DESIGN.md) 的「貫穿全專案的設計原則」第 6 條，含一條我們自己學到的**例外**：
被截的若是**識別字**（`name` 兼 `[[link]]` 目標），加標記會破壞用途，改成「切在語意邊界並保持自洽」。

**全庫上限審計**（只列會被人或模型看到的；純內部佇列/計時不算）：

| 輸出點 | 上限 | 狀態 |
|---|---|---|
| `read_file` | 8000 字元 | ✅ 明講「已被系統截斷至前 N 字元」 |
| 使用者畫像常駐 | 2000 字 | ✅ 「另有 N 條…需要時用 recall 取回」 |
| **KG 子圖** | 8 節點 | ✅ **本次修掉**——`Subgraph` 回傳截斷旗標，`RecallGraph` 附上「仍有未展開的鄰居」 |
| **`search_sessions`** | limit 硬夾到 20 | ⬜ **未修**：呼叫端要 50 會靜默拿到 20，且不回報符合總數 |
| 記憶 `name` | 24 字 | ✅ 走例外條款（切在標點強度分層邊界），非加標記 |

`search_sessions` 那條留著：改動要動工具的回傳結構（多一個總數欄位），範圍比其餘幾項大，
且它的消費者是模型不是人，優先級低。
