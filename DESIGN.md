...
6. **絕不靜默截斷**。任何上限——字數、條數、跳數、佇列——都要在輸出處講出來：字串截斷帶可見標記（`…` 或「已截斷至前 N 字元」），清單封頂講「另有 N 條」。看不見的截斷比看得見的短更糟：讀的人（或模型）會把殘缺當成全部，而且事後查不出來。
   
   **識別字是例外，但方向相反**：當被截的東西是 ID（記憶記錄的 `name` 兼 `[[link]]` 目標），加標記會破壞它的用途——那就改成**切在語意邊界**並保持自洽（切在標點、剝掉未閉合的記號與括號）。實例見 [kg-status.md](docs/kg-status.md) §4：`name` 硬切 24 字的結果是 14 個節點全斷在句子中間、圖一條人工邊都長不出來，而且沒有任何地方顯示「這是被截斷的」。
...
- **技能也有行為評測**（[eval/skillab.go](internal/eval/skillab.go)，`cmd/bench -skill-ab`）：skillgate 只做**結構把關**（安全/格式），無法回答「這技能到底有沒有讓任務更好」。故加技能版 L2 A/B——同一任務在「無技能 vs 綁定技能」下各跑一次比回合/成本/通過。內建情境是「升版本的隱藏雙檔慣例」（改版須同時改 `VERSION` 與 `package.json` 且一致，這條**無法從檔案內容推得**）：無技能常漏改一檔而驗證失敗、或需多回合探索；綁技能則直接照做。把「技能有沒有用」從感覺變成可量測。
