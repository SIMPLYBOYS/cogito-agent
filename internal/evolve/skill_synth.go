...
// reflectSystemPrompt 是技能萃取的系統提示。
//
// 「每步要有完成條件」與「刪掉不改變行為的句子」兩條取自 mattpocock/skills 的
// writing-for-agents。它本身是一份【技能】，但在我們這裡當技能是錯的：技能索引進每一輪
// prompt，一份「怎麼寫技能」的技能會一直收費卻幾乎不被觸發。它該在的地方是這裡——
// 技能實際被寫出來的那一刻。
//
// 刻意【沒有】採用它的「避免否定句、改寫成正向目標」：skill_gate.go 的 negationPattern
// 已經反向裁決過同一題——一份好技能本來就會把踩過的雷寫進去，逼作者不准提才是真的會漂。
...
- **刪掉不改變行為的句子。** 每寫一句就自問：拿掉它，執行者的行為會不一樣嗎？
  「請仔細思考」「務必確保品質」「注意細節」這類話跟預設行為【沒有差別】，
  它們只是佔 context。技能的價值不是長度，是「改變行為的句子」佔的比例。
...
1. 具體、命令層級的步驟（不要寫死本次的檔名/數值）。
   **每一步都要寫「怎麼算做完」**——一個看得見的結果：某個檔案存在、某個指令 exit 0、
   輸出裡含某個字串、某個數字降到多少以下。沒有完成條件的步驟，執行者只能【猜】自己做完了沒，
   於是不是提早收手就是無限打磨；而且下一步該不該開始也沒有依據。
## Examples
- 真實範例：命令 + 預期輸出
