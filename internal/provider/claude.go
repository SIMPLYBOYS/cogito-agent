...
// 不覆蓋 baseURL，讓 SDK 使用預設地址；憑證讀取 ANTHROPIC_API_KEY。
...
func (p *ClaudeProvider) buildParams(msgs []schema.Message, availableTools []schema.ToolDefinition) anthropic.MessageNewParams {
	...
	for _, toolDef := range availableTools {
		// ToolInputSchemaParam 是結構體，需要通過 Properties 欄位填充
...
//   - 緊跟 tool_result 之後的普通 user 文本（如無窮迴圈提醒）併入同一條 user 消息（文本塊），
...
func buildAnthropicMessages(msgs []schema.Message) ([]anthropic.MessageParam, string) {
	...
	// 迴圈結束後把殘留的工具結果 flush 掉（最後一條消息正好是 tool_result 的情況）
	flushToolResults()
