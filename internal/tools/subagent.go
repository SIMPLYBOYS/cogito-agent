...
type AgentRunner interface {
	RunSub(ctx context.Context, taskPrompt string, skillBody string, readOnlyRegistry Registry, reporter interface{}) (string, error)
}
...
// skillsBaseDir 是含 .claw/skills 的目錄（須與主 agent 的技能索引同源）。傳空字串則無可綁定技能。
...
func (t *SubagentTool) Definition() schema.ToolDefinition {
	return schema.ToolDefinition{
		Name:        t.Name(),
		Description: "派出一個專門用於深度探索（Exploration）的子智能體。當你需要閱讀大量代碼、跨文件查找邏輯時請調用此工具。它在探索完畢後，會給你返回一份極度精煉的摘要報告。可選 skill 參數：綁定一個技能後，該技能的完整正文只載入子智能體的隔離 context（主 context 不被汙染），適合需要長篇操作指南的任務。",
...
func (t *SubagentTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	summary, err := t.runner.RunSub(ctx, input.TaskPrompt, skillBody, t.readOnlyRegistry, t.reporter)
	if err != nil {
