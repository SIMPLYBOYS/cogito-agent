...
// 死循環探測雙閾值：
//   - fingerprintThreshold：完全相同 (工具名+參數) 連續失敗——典型的原地踏步死循環。
//   - sameToolThreshold：同一工具(任意參數)連續失敗——攔截「每次微調參數盲目試錯」這種
//     會繞過指紋探測的失控重試（閾值略高，給正常的多文件探索留餘量）。
const (
	fingerprintThreshold = 3
	sameToolThreshold    = 5
)

// ReminderInjector 是死循環探測器：同時按 (工具名+參數) 指紋與 (工具名) 兩個維度統計連續
// 失敗次數，任一維度達閾值就注入一條強力"跳出循環、換策略"的提醒，避免模型盯著同一個錯誤
...
// CheckTurn 對本輪所有工具的調用與結果按順序更新計數，任一達閾值即返回一條提醒
...
// ≥sameToolThreshold 次，則返回一條強提醒消息（否則返回 nil）。
func (r *ReminderInjector) CheckAndInject(lastToolCall schema.ToolCall, lastResult schema.ToolResult) *schema.Message {
	...
	if sameArgsLoop {
		diagnosis = fmt.Sprintf("你剛剛連續 %d 次使用完全相同的參數調用了 '%s' 工具，並且都失敗了。", fpCount, lastToolCall.Name)
	} else {
		diagnosis = fmt.Sprintf("你已經連續 %d 次調用 '%s' 工具（即便每次都更換了參數）並且全部失敗——不斷微調參數的盲目試錯同樣是死循環。", toolCount, lastToolCall.Name)
	}
...
3. 如果你確實無法通過系統工具解決當前問題，請直接結束任務並向用戶說明你需要什麼人工幫助，而不是繼續盲目消耗 API 資源嘗試。`, diagnosis)
