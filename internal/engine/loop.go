...
// 主迴圈硬性防線的預設值：兩者都是框架層強制（不依賴模型自覺），<=0 表示關閉該防線。
const (
...
// compactor —— 每次發 LLM 前做字元級壓縮（OOM 防線）。
...
type AgentEngine struct {
	...
	EnableThinking bool
	PlanMode       bool
	MaxTurns       int     // 主迴圈硬性回合上限（防失控重試燒 API）；<=0 不限制
	MaxCostUSD     float64 // 單次 Run 的成本熔斷上限（USD）；<=0 不限制
	// 單輪內工具的最大同時併發數（信號量）；<=0 不限制。防模型一次吐大量工具請求時瞬間打爆
	// 下游（如網路工具撞 Rate Limit）。採 per-turn 而非 registry 全域，以避開 spawn_subagent
...
func NewAgentEngine(p provider.LLMProvider, r tools.Registry, enableThinking bool, planMode bool) *AgentEngine {
	return &AgentEngine{
		...
		// 自適應壓縮：水位線按 provider 的真實上下文窗口（token）設定，而非寫死字元數；
		// watermark 傳 0 → 用預設 75%。壓縮器再以每輪真實 PromptTokens 自校準 byte/token 比。
...
func (e *AgentEngine) Run(ctx context.Context, session *ctxpkg.Session, reporter Reporter) error {
	...
	for {
		...
		if e.MaxCostUSD > 0 {
			...
			if !softLanded && spent > e.MaxCostUSD*costSoftLandingRatio {
				...
				session.Append(schema.Message{
					...
					// 措辭要精準：先前寫「立即停止呼叫任何工具」＋「交付要含產物路徑」，這兩句是矛盾的——
					// 產物還沒寫檔的模型只能【編一個路徑】出來。實際踩到：一場會議的逐字稿與規格被貼在
					// 對話裡，同時宣稱「我已寫入以下兩檔」，磁碟上一個檔都沒有。
					// 所以：探索類的停、落地類的放行，而且明確要求區分「已落地」與「未落地」。
					Content: fmt.Sprintf("[系統提醒] 本次任務預算即將用盡（已花 $%.2f / 上限 $%.2f）。請【停止一切探索與驗證類的工具呼叫】（讀檔、搜尋、跑指令、再問子 agent）。\n"+
						"但【尚未落地的產物必須先寫檔】：只寫檔，不要順便讀回來確認。\n"+
						"然後給最終交付：產物路徑只列你【確實寫成功】的；還沒寫成檔案的內容，直說「未落地」並把內容附在回覆裡——不要宣稱磁碟上有不存在的檔案。", spent, e.MaxCostUSD),
				})
			}
		}
		...
		// defer 綁函式回傳，會把所有回合 span 拖到整個 Run 結束才一起關，導致每回合耗時被記成
		...
		// 【核心防線】發 LLM 前做字元級壓縮；只動發給 LLM 的副本，不損毀 session.history。
		...
		// Phase 1: Thinking
		// 注意：手動兩階段思考（剝奪 tools）對 Claude 會退化成 <invoke> 文本，故各入口預設
		...
		// 收集本輪【所有】工具的呼叫與原始結果（按 idx 各寫各的槽，無 data race），
		// 供 ReminderInjector 做無窮迴圈分析——並行呼叫的每一個都要納入，不再只看第一個。
...
// RunSub 是為 SubagentTool 拉起的一次性、受限的 ReAct 迴圈：
//   - 不依賴外部 Session，對話歷史是局部變數，跑完即丟（上下文隔離的關鍵）；
//   - 工具集僅為 caller 傳入的 readOnlyRegistry（能力沙箱）；
//   - 強制關閉慢思考，直接行動；有 maxSubTurns 硬上限防卡死；
//   - 回傳值 string 即"探索報告"，作為 spawn_subagent 工具的輸出回給主 agent。
//
// 滿足 tools.AgentRunner 介面；reporter 用 any 規避包依賴，內部斷言回 Reporter。
func (e *AgentEngine) RunSub(ctx context.Context, task tools.SubTask) (string, error) {
	...
	if model != "" || task.MaxTokens > 0 {
		if cfg, ok := e.provider.(provider.Configurable); ok {
			prov = cfg.Configure(model, task.MaxTokens)
...
你的任務是根據主架構師的指令，在當前工作區內仔細閱讀程式碼、查閱日誌，蒐集足夠的訊息。

【核心紀律】
1. 你必須、且只能依靠內置工具（如 bash 的 find/grep，或 read_file）去尋找答案。絕對不允許憑空捏造或猜測！
2. 如果你沒有找到確切的答案，你必須繼續使用工具深入搜索。
3. 當且僅當你找到了確切的線索後，停止呼叫工具，直接輸出一段純文本作為你的終極彙報。主架構師會根據你的彙報來做下一步決策。`
	}
	...
	for {
		...
		if turnCount > maxSubTurns {
			return "", fmt.Errorf("子 agent探索過於深入，超過 %d 輪被強制召回，請主 Agent 給它更明確的指令", maxSubTurns)
		}

		// 【駕馭底線】子 agent僅能使用傳入的只讀工具註冊表（無 spawn_subagent → 無遞迴）
		...
		// 退出條件：不再呼叫工具 → 它已寫好報告，直接把 Content 當回傳值給主 agent
		if len(actionResp.ToolCalls) == 0 {
